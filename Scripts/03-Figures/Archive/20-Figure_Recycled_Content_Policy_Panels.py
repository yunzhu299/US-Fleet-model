#!/usr/bin/env python3
"""Maximum recycled-content potential across policy and technology scenarios."""
...
STEM = OUT / "Figure6_Maximum_Recycled_Content_Baseline_vs_Rollback"
...
def load_policy(folder: str) -> pd.DataFrame:
    data = pd.read_csv(DATA_ROOT / folder / "_parity" / "ratio_results.csv")
...
fig, axes = plt.subplots(
    2, 6, figsize=(15.8, 7.4), sharex=True, sharey=True,
...
fig.text(0.055, 0.925, "Baseline Policy", ha="left", fontsize=14, fontweight="bold")
fig.text(0.545, 0.925, "Rollback Policy", ha="left", fontsize=14, fontweight="bold")
fig.supylabel("Maximum recycled content (%)", x=0.012, fontsize=13, fontweight="bold")
...
fig.add_artist(
    Line2D([0.518, 0.518], [0.245, 0.89], transform=fig.transFigure,
