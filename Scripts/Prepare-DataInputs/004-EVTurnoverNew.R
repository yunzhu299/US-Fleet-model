...
#    - NEW: `reuse_shrink` (0..1) scales domestic LIB reuse to reflect exports
# -----------------------------
EV_engine_step <- function(engine, sales_y = 0,
                           ...
                           max_ev_age     = 20,
                           max_lib_age_ev = 12,
                           reuse_shrink   = 1.0) {  # 1.0 = no export; <1 = some reuse lost to exports
  ...
  # offset reuse to align with EV ages that need a battery
  ...
  LIB_reuse_vector <- as.integer(reuse_offset_vec + reuse_loop_vec)  # by EV age
  LIB_reuse_EV     <- sum(LIB_reuse_vector)
  
  # new LIBs required (unmet needs after reuse)
  LIB_newadd_vector <- as.integer(ev_need_vec)   # by EV age
  LIB_new_add       <- sum(LIB_newadd_vector)
  
  # --- NEW: apply export adjustment to reuse ---
  # Keep only a share of domestic reuse, convert the lost share to new LIB demand
  ...
  if (reuse_shrink < 1) {
    ...
    if (sum(lost_reuse_vec) > 0) {
      ...
      # also add per-age new LIBs into column 1 of the matrix
  ...
  lib_available_vec  <- lib_available_vec + lib_to_EV_vec
  new_matrix[,1]     <- new_matrix[,1] + as.integer(ev_need_vec)  # unchanged part
