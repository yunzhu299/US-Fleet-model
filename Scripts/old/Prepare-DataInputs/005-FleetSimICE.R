...
# Normalize EV_historical column names defensively
if (!("State" %in% names(EV_historical)) && ("state" %in% names(EV_historical))) {
  EV_historical <- EV_historical %>% rename(State = state)
}
if (!("Global Segment" %in% names(EV_historical)) && ("Segment" %in% names(EV_historical))) {
  EV_historical <- EV_historical %>% rename(`Global Segment` = Segment)
}
if (!("Sale Year" %in% names(EV_historical)) && ("Sale_Year" %in% names(EV_historical))) {
  EV_historical <- EV_historical %>% rename(`Sale Year` = Sale_Year)
}
if (!("Sales" %in% names(EV_historical)) && ("Total Sales" %in% names(EV_historical))) {
  EV_historical <- EV_historical %>% rename(Sales = `Total Sales`)
}
required_ev_cols <- c("State", "Sale Year", "Global Segment", "Propulsion", "Sales")
missing_ev_cols <- setdiff(required_ev_cols, names(EV_historical))
if (length(missing_ev_cols) > 0) {
  stop("EV_historical missing required columns: ", paste(missing_ev_cols, collapse = ", "))
}
...
run_one_scenario <- function(PR_table, scenario_tag = "ACCII") {
  ...
  ice_stock_rows <- list() # State x Segment x Year x Age ICE stock
  ...
  for (yr in years) {
    ...
    # record end-of-year ICE stock vector by age (0-49)
    ice_stock_rows[[as.character(yr)]] <- purrr::map_dfr(seq_len(nrow(ice_keys)), function(i) {
      k <- ice_keys[i, ]
      key <- paste(k$State, k$Segment, sep = " | ")
      N <- as.integer(ice_env[[key]])
      tibble(
        State = k$State,
        Segment = k$Segment,
        Year = yr,
  ...
  # 6) Outputs
  # -----------------------------
  # ---- ICE stock vectors by State x Segment x Year x Age (0-49)
  ice_stock_by_age <- if (length(ice_stock_rows)) {
    bind_rows(ice_stock_rows) %>% arrange(State, Segment, Year, Age)
  } else {
    tibble(
      State = character(),
      Segment = character(),
  ...
  write.csv(
    ice_stock_by_age,
    paste0("Outputs/ICE_Stock_Vector_byStateSegAge_", scenario_tag, ".csv"),
    row.names = FALSE
  )
  ...
  invisible(list(
    ...
    ice_stock_by_age = ice_stock_by_age
  ))
}
