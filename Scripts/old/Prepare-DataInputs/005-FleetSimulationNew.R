...
run_one_scenario <- function(PR_table, scenario_tag = "ACCII") {
  ...
  ice_stock_rows <- purrr::map_dfr(seq_len(nrow(ice_keys)), function(i) {
    k <- ice_keys[i, ]
  ...
  if (!exists("ice_stock_all_years", inherits = FALSE)) {
    ice_stock_all_years <- list()
  }
  ice_stock_all_years[[as.character(yr)]] <- ice_stock_rows
  ...
  # ---- ICE stock by age table
  ice_stock_by_age <- if (exists("ice_stock_all_years", inherits = FALSE) && length(ice_stock_all_years)) {
    bind_rows(ice_stock_all_years) %>% arrange(State, Segment, Year, Age)
  } else {
    tibble(State = character(), Segment = character(), Year = integer(), Age = integer(), ICE_stock_vector = integer())
  }
  write.csv(
    ice_stock_by_age,
