...
ev_hist_file <- if (file.exists("Inputs/historical_state_pt_veh_df.csv")) {
  "Inputs/historical_state_pt_veh_df.csv"
} else if (file.exists("Outputs/EV_historical_new.csv")) {
  "Outputs/EV_historical_new.csv"
} else {
  "~/Downloads/historical_state_pt_veh_df.csv"
}
EV_historical <- read_csv(ev_hist_file, show_col_types = FALSE) %>%
  select(`Sale Year`, State, Propulsion, `Global Segment`, Sales)

sales_2020_25_file <- if (file.exists("Inputs/2020-2025sales.xlsx")) {
  "Inputs/2020-2025sales.xlsx"
} else {
  "~/Downloads/2020-2025sales.xlsx"
}
X2020_2025sales <- readxl::read_excel(sales_2020_25_file, sheet = "Sheet1")
...
# Lifetime params by segment (SUV battery mean changed to 15, same as Car)
life_param <- tibble(
  Vehicle  = c("Car","SUV"),
  mean_ev  = c(17,17),
  sd_ev    = c(4,4),
