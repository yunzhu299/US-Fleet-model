# ==============================================================================
# Script: 41-BESS_Second_Life.R
# Purpose:
#   Run all LDV and HDV second-life BESS stock and retirement simulations
#   for the United States, Canada, and Mexico under ACCII and Repeal.
#
# Modeling choice:
#   Inflow enters at BEGINNING of year, then retires within the SAME year.
#
# IMPORTANT (match 04-EV_Turnover.R battery assumptions):
#   dist: Logistic
#   mean_lib = 20, sd_lib = 4
...
message("\nLDV BESS simulations complete; starting HDV BESS...")

# ------------------------------------------------------------------------------
# 7) HDV BESS
# ------------------------------------------------------------------------------

run_hdv_bess <- function(scenario) {
  input_file <- file.path(
    "Outputs", "HDV", paste0("HDV_EV_Turnover_", scenario, ".csv")
  )
  if (!file.exists(input_file)) {
    stop("Missing HDV turnover input: ", input_file,
         ". Run 31-HDV_Turnover.R before 41-BESS_Second_Life.R.")
  }

  hdv_raw <- read_csv(input_file, show_col_types = FALSE) %>%
    mutate(
      Year = as.integer(Year),
    ...
    filter(Year >= 2022, Year <= END_YEAR)

  hdv_groups <- hdv_raw %>%
    group_by(Country, State, Vehicle) %>%
    group_split()

  res <- map(hdv_groups, function(g) {
    keys <- g %>% distinct(Country, State, Vehicle) %>% slice(1)
    sim <- simulate_one_group(g)
    out_s <- sim$stock
    out_r <- sim$retire
    if (nrow(out_s) > 0) out_s <- out_s %>% filter(Year >= 2022)
    if (nrow(out_r) > 0) out_r <- out_r %>% filter(Year >= 2022)

    if (nrow(out_s) == 0) {
      out_s <- tibble(
        Year = 2022:END_YEAR,
        BESS_stock_vector = paste(rep(0, length(AGE_BINS)), collapse = "|"),
        BESS_stock_total = 0L
      )
    }
    if (nrow(out_r) == 0) {
      out_r <- tibble(
        Year = 2022:END_YEAR,
        BESS_retire_vector = paste(rep(0, length(AGE_BINS)), collapse = "|"),
        BESS_retire_total = 0L
      )
    }

    list(
      stock = bind_cols(keys[rep(1, nrow(out_s)), , drop = FALSE], out_s),
      retire = bind_cols(keys[rep(1, nrow(out_r)), , drop = FALSE], out_r)
    )
  })

  stock_all <- bind_rows(map(res, "stock"))
  retire_all <- bind_rows(map(res, "retire"))
  write_csv(
    stock_all,
    file.path("Outputs", "HDV", paste0("HDV_BESS_Stock_", scenario, ".csv"))
  )
  write_csv(
    retire_all,
    file.path("Outputs", "HDV", paste0("HDV_BESS_Retire_", scenario, ".csv"))
  )
}

walk(SCENARIOS, run_hdv_bess)
message("All LDV and HDV BESS simulations complete.")
