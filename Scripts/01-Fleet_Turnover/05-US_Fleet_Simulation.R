...
## Uses ICE survival from 02 and EV engine from 04 (Logistic + LIB reuse/repurpose/recycling)
...
##          (does NOT change vehicle stock; ONLY adjusts LIB reuse/repurpose/recycle/new-add flows)
## YZC Oct 2025 + updated for BESS (Nov 2025)
...
# -----------------------------
# The table is derived internally after turnover is complete; no stale output
# file or global ratio object is required.
get_export_factor <- function(yr, export_factor_tbl = NULL) {
  ef <- 0
  if (!is.null(export_factor_tbl)) {
    r <- export_factor_tbl %>%
      filter(Year == yr) %>%
      pull(export_factor)
...
run_one_scenario <- function(PR_table, scenario_tag = "ACCII",
                             export_factor_tbl = NULL,
                             write_outputs = TRUE) {
  flows_need_post_adjust <- is.null(export_factor_tbl)
  ...
  for (yr in years) {
    ...
    export_factor_y <- get_export_factor(yr, export_factor_tbl)
      ...
      for (st in unique(ev_real$State)) {
        for (seg in c("Car","SUV")) {
          for (pp in c("BEV","PHEV")) {
            sales_y <- ev_real %>%
              filter(State==st, Segment==seg, Propulsion==pp) %>%
              summarise(val = sum(Sales, na.rm = TRUE)) %>%
              pull(val)
            ...
            # Domestic LIB flows (after exports)
            LIB_recycling_vec_adj <- scale_int(step2$LIB_recycling_vector, domestic_factor)
            LIB_available_vec_adj <- scale_int(step2$LIB_available_vector, domestic_factor) # repurpose
            ...
            # Reuse lost due to export → extra new LIB demand
            lost_reuse <- sum(as.integer(step2$LIB_reuse_vector)) -
              sum(LIB_reuse_vec_adj)
            LIB_newadd_vec_adj <- as.integer(step2$LIB_newadd_vector)
            LIB_newadd_vec_adj <- add_proportional(
              LIB_newadd_vec_adj,
            ...
            evlib_rows[[length(evlib_rows) + 1]] <- tibble(
              ...
              LIB_available  = as.integer(sum(LIB_available_vec_adj)),   # = repurpose
              LIB_reuse_EV   = as.integer(sum(LIB_reuse_vec_adj)),
              LIB_new_add    = as.integer(sum(LIB_newadd_vec_adj)),
              LIB_repurpose  = as.integer(sum(LIB_repurpose_vec_adj)),   # explicit repurpose
  ...
  # Export does not change turnover: derive the factor from the completed
  # retirements, then apply it to flows from this same pass.
  if (is.null(export_factor_tbl)) {
    prelim_totals <- result_add_and_retire %>%
      group_by(State, Year) %>%
      summarise(
    ...
    export_factor_tbl <- derive_export_factor_table(prelim_totals)
  }

  result_add_and_retire <- result_add_and_retire %>%
    select(-exp_ICE, -exp_BEV, -exp_PHEV) %>%
    left_join(export_factor_tbl, by = "Year") %>%
    mutate(
      export_factor = coalesce(export_factor, 0),
      exp_ICE = export_factor * ret_ICE,
      exp_BEV = export_factor * ret_BEV,
      exp_PHEV = export_factor * ret_PHEV
    ) %>%
    select(-export_factor)
  ...
  evlib_detail <- if (length(evlib_rows)) bind_rows(evlib_rows) else
    tibble(
      ...
      LIB_reuse_EV=double(), LIB_new_add=double(), LIB_repurpose=double(),
      EV_stock=double(),
      LIB_recycling_vector=list(), LIB_available_vector=list(),
      LIB_reuse_vector=list(), LIB_newadd_vector=list(),
      LIB_repurpose_vector=list(), EV_stock_vector=list(),
      domestic_factor=double(), export_factor=double()
    )

  if (flows_need_post_adjust && nrow(evlib_detail) > 0) {
    factor_by_year <- export_factor_tbl$export_factor
    names(factor_by_year) <- as.character(export_factor_tbl$Year)
    for (i in seq_len(nrow(evlib_detail))) {
      ef <- factor_by_year[[as.character(evlib_detail$Year[i])]]
      if (is.null(ef) || !is.finite(ef)) ef <- 0
      domestic <- 1 - ef

      raw_reuse <- evlib_detail$LIB_reuse_vector[[i]]
      reuse_adj <- scale_int(raw_reuse, domestic)
      recycle_adj <- scale_int(evlib_detail$LIB_recycling_vector[[i]], domestic)
      available_adj <- scale_int(evlib_detail$LIB_available_vector[[i]], domestic)
      repurpose_adj <- scale_int(evlib_detail$LIB_repurpose_vector[[i]], domestic)
      newadd_adj <- as.integer(evlib_detail$LIB_newadd_vector[[i]])
      lost_reuse <- sum(as.integer(raw_reuse)) - sum(reuse_adj)
      newadd_adj <- add_proportional(newadd_adj, raw_reuse, as.integer(lost_reuse))

      evlib_detail$LIB_reuse_vector[[i]] <- reuse_adj
      evlib_detail$LIB_recycling_vector[[i]] <- recycle_adj
      evlib_detail$LIB_available_vector[[i]] <- available_adj
      evlib_detail$LIB_repurpose_vector[[i]] <- repurpose_adj
      evlib_detail$LIB_newadd_vector[[i]] <- newadd_adj
      evlib_detail$LIB_reuse_EV[i] <- sum(reuse_adj)
      evlib_detail$LIB_recycling[i] <- sum(recycle_adj)
      evlib_detail$LIB_available[i] <- sum(available_adj)
      evlib_detail$LIB_repurpose[i] <- sum(repurpose_adj)
      evlib_detail$LIB_new_add[i] <- sum(newadd_adj)
      evlib_detail$export_factor[i] <- ef
      evlib_detail$domestic_factor[i] <- domestic
    }
  }
  ...
  exports_yearly <- result_add_and_retire %>%
    group_by(Year) %>%
    summarise(
      Export_ICE = sum(exp_ICE, na.rm = TRUE),
      Export_BEV = sum(exp_BEV, na.rm = TRUE),
...
# -----------------------------
# Derive annual export factors from completed turnover in the same pass.
# -----------------------------
derive_export_factor_table <- function(prelim_totals) {
  retire_nat <- prelim_totals %>%
    filter(Year >= 2020, Year <= 2024) %>%
    group_by(Year) %>%
    summarise(
      Ret_ALL = sum(ret_ICE + ret_BEV + ret_PHEV, na.rm = TRUE),
      .groups = "drop"
    )

  hist <- net_export_by_year %>%
    select(Year, Net_Export) %>%
    left_join(retire_nat, by = "Year") %>%
    mutate(
      Ret_ALL = pmax(coalesce(Ret_ALL, 0), 1),
      export_factor = pmin(1, pmax(0, coalesce(Net_Export, 0) / Ret_ALL))
    )

  future_factor <- with(
    hist,
    sum(export_factor * Ret_ALL, na.rm = TRUE) / sum(Ret_ALL, na.rm = TRUE)
  )
  if (!is.finite(future_factor)) future_factor <- 0

  tibble(Year = years) %>%
    left_join(hist %>% select(Year, export_factor), by = "Year") %>%
    mutate(export_factor = if_else(Year >= 2025, future_factor,
                                   coalesce(export_factor, 0)))
}
